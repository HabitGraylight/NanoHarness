"""Portable, deterministic scenarios shared by gallery harnesses."""
...
class Scenario(BaseModel):
    ...
    schema_version: str = GALLERY_SCENARIO_VERSION
    ...
    def supported_version(cls, value: str) -> str:
        if value != GALLERY_SCENARIO_VERSION:
            raise ValueError(
                f"unsupported gallery scenario version {value!r}; "
                f"expected {GALLERY_SCENARIO_VERSION!r}"
            )
        return value
...
def load_scenario(path: str | Path) -> Scenario:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Gallery scenario must contain a YAML object")
